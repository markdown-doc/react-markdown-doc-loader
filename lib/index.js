...
function prepareAST(sections) {
  ...
  var docSections = sections.map(function(s) {
    if (s.type === 'style') {
      return helper.createRawHtmlRenderer('style', s.value);
    } else if (s.type === 'markdown') {
      return helper.createRawHtmlRenderer('div', s.value);
    } else if (s.type === 'demo') {
      return helper.createDemoRenderer(s.title, s.src, s.id);
    }
  });
  ...
  return {
    ...
    // SECTIONS: helper.createDocContainerElement(docSections)
  };
}
